...
static void layer_update_cb(struct layer_state state) {
    ...
    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) {
        ...
        // Re-tint the layer label to match the new layer's accent.
...
int zmk_widget_wpm_meter_init(struct zmk_widget_wpm_meter *widget, lv_obj_t *parent) {
    ...
    // bg punches through any bar underneath; was 0x000000 which leaked black
    // against the Nord backdrop. Now matches the screen Nord0 bg.
    ...
    // Same Nord0 bg fix as wpm_label.
    lv_obj_set_style_bg_color(widget->layer_label, lv_color_hex(DISPLAY_COLOR_BG), LV_PART_MAIN);
