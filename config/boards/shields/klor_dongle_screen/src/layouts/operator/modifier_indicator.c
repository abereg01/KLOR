...
static void set_modifier_color(lv_obj_t *label, enum modifier_type type, bool active) {
    lv_color_t color = active ? lv_color_hex(display_color_for_mod(type))
...
#endif
            set_modifier_color(widget->mod_labels[i], type, state.mods[type]);
        }
    }
}
